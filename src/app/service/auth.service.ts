...
export class AuthService {
  private users = [
    ...
    {email:'tristan@gmail.com'  , password:'123'  , roles:['USER']},
  ];
  ...
  getUser(){
    ...
    if(user){ //si j'ai déjà un utilisateur en LS, c'est que je suis connecté
      this.userConnected = JSON.parse(user);    // décryptage
    }
  ...
  login(email: string, password: string) {
    ...
    this.users.forEach( (user) => {
        if((user.email == email) && (user.password == password)){
          ...
          localStorage.setItem('user',JSON.stringify(user));  //cryptage des données avant stockage en LS
        }
    });
    return connected;
  }
